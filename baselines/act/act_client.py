...
class RequestMessage:
    ...
    def serialize(self):
        ...
        msg = {
            ...
            "state": self.state,
            "condition": {},  # Not used by ACT but required by server
...
def main():
    parser = argparse.ArgumentParser(description="Act Client for testing Action Chunking Transformer server")
    ...
    parser.add_argument("--dataset-path", type=str, 
                        default="/hfm/data/real_teleop_g1/lerobot/Pick_bottle_and_turn_and_pour_into_cup",
    ...
    # 1. Create client
    client = ACTActionClient(args.ip, args.port)
    
    # Health check
    print(f"Connecting to server at {args.ip}:{args.port}...")
    if not client.health_check():
        print("❌ Server is not responding. Please start act_serve.py first.")
        return
