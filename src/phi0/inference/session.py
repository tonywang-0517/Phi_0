...
def resolve_deploy_action_chunk_size(
    ...
    *,
    ...
    """Training-aligned deploy horizon: ``seq_len - past_action_window_size``."""
    w = int(getattr(model, "past_action_window_size", 0))
    if seq_len is None:
        seq_len = 33
    ...
    def get_or_build(
        ...
        *,
        ...
        if model.uses_dual_vggt_cross_attn() and "video" in inputs:
            vggt_ctx, vggt_ctx_mask = model._resolve_vggt_context(inputs["video"])
...
class ActionInferenceSession:
    """Stateful deploy: Cosmos multi-frame hook + proprio history + chunk predict."""
    ...
    def __init__(
        ...
        *,
        ...
        self._proprio_history: Deque[torch.Tensor] = deque(
    ...
    def _update_action_context_from_video(self, video: torch.Tensor) -> None:
        ...
        latents = self.model._encode_video_latents(video)
        ...
        if getattr(tower, "transformer", None) is not None:
            ...
            self.action_ctx, self.action_ctx_mask = tower.extract_action_context(
                latents, self._prompt_embeds
            )
        if self.model.uses_dual_vggt_cross_attn():
            self.vggt_ctx, self.vggt_ctx_mask = self.model._resolve_vggt_context(video)
        else:
            self.vggt_ctx = None
            self.vggt_ctx_mask = None
    ...
    def prefill_from_clip_inputs(self, inputs: Dict[str, Any]) -> None:
        ...
        if self.model.uses_dual_vggt_cross_attn():
            if "vggt_ctx" in inputs and "vggt_ctx_mask" in inputs:
                ...
                self.vggt_ctx, self.vggt_ctx_mask = self.model._resolve_vggt_context(inputs["video"])
            else:
    ...
    def predict(self, num_frames: int, *, denormalize: bool = False) -> torch.Tensor:
        ...
        pred = self.model.predict_action(
            ...
            proprio_tokens=self._proprio_tensor(),
        )
