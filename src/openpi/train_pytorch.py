...
def train_loop(config: _config.TrainConfig):
    ...
    if config.pytorch_weight_path is not None:
        ...
        target_action_dim = int(config.model.action_dim)
        while state_dict["action_in_proj.weight"].shape[1] < target_action_dim:
            cur_dim = state_dict["action_in_proj.weight"].shape[1]
            pad_dim = target_action_dim - cur_dim
            w = state_dict["action_in_proj.weight"]
            to_pad = w[:, -min(pad_dim, cur_dim) :]
            if to_pad.shape[1] < pad_dim:
                reps = (pad_dim + to_pad.shape[1] - 1) // to_pad.shape[1]
                to_pad = to_pad.repeat(1, reps)[:, :pad_dim]
            state_dict["action_in_proj.weight"] = torch.cat([w, to_pad], dim=1)

            b = state_dict["action_out_proj.bias"]
            b_pad = b[-min(pad_dim, b.shape[0]) :]
            if b_pad.shape[0] < pad_dim:
                reps = (pad_dim + b_pad.shape[0] - 1) // b_pad.shape[0]
                b_pad = b_pad.repeat(reps)[:pad_dim]
            state_dict["action_out_proj.bias"] = torch.cat([b, b_pad], dim=0)

            w = state_dict["action_out_proj.weight"]
            w_pad = w[-min(pad_dim, w.shape[0]) :, :]
            if w_pad.shape[0] < pad_dim:
                reps = (pad_dim + w_pad.shape[0] - 1) // w_pad.shape[0]
                w_pad = w_pad.repeat(reps, 1)[:pad_dim, :]
            state_dict["action_out_proj.weight"] = torch.cat([w, w_pad], dim=0)
