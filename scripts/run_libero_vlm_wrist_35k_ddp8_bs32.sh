#!/usr/bin/env bash
# Wrist + VLM: 8-GPU DDP, bs=32/GPU (eff=256), lr=1e-4, 35k steps.
set -euo pipefail
...
EXP="${EXP:-experiments/libero_spatial_vlm_wrist_35k_ddp8_bs32}"
CONFIG="${CONFIG:-train_libero_spatial_vlm_wrist_35k_ddp8_bs32}"
CKPT_NAME="${CKPT_NAME:-libero_spatial_vlm_wrist_35k_ddp8_bs32}"
...
# Always honor CUDA_DEVICES; do not inherit a stale CUDA_VISIBLE_DEVICES from the shell.
...
echo "==> VLM+wrist 35k DDP8 bs32: ${NGPUS} GPUs (${CUDA_DEVICES})"
...
echo "    memory: num_workers=2/rank, cache=auto-off at 8-GPU scale (~480GiB vs ~2TiB)"
