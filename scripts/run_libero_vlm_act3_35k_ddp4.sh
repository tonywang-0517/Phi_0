#!/usr/bin/env bash
# Ablation E: VLM + ACT head 3 layers (0.5x baseline), mono camera, 4-GPU DDP, 35k steps, lr=1e-4.
set -euo pipefail
...
EXP="${EXP:-experiments/libero_spatial_vlm_act3_35k_ddp4}"
CONFIG="${CONFIG:-train_libero_spatial_vlm_act3_35k_ddp4}"
CKPT_NAME="${CKPT_NAME:-libero_spatial_vlm_act3_35k_ddp4}"
...
echo "==> VLM+ACT3 (0.5x blocks) 35k DDP: ${NGPUS} GPUs (${CUDA_DEVICES})"
