#!/usr/bin/env bash
# Multi-GPU DDP training for LIBERO delta-EEF (Psi0 finetune-simple aligned).
set -euo pipefail
...
EXP="${EXP:-experiments/libero_spatial_vlm_dual_15k}"
CONFIG="${CONFIG:-train_libero_spatial_act_delta_15k}"
CKPT_NAME="${CKPT_NAME:-libero_spatial_vlm_dual_15k}"
# Psi0: train_batch_size is per-GPU.
...
echo "==> Psi0-style DDP: ${NGPUS} GPUs (${CUDA_DEVICES})"
