#!/usr/bin/env bash
# 4-GPU DDP: pick_tissue_valid 512-d unified finetune (configs/train_pick_tissue_finetune_ddp4.yaml).
set -euo pipefail
...
EXP="${EXP:-experiments/pick_tissue_valid_wholebody_best_8k_ddp4}"
CONFIG="${CONFIG:-train_pick_tissue_finetune_ddp4}"
CKPT_NAME="${CKPT_NAME:-pick_tissue_valid_wholebody_best_act}"
...
echo "==> Pick-tissue valid finetune DDP: ${NGPUS} GPUs (${CUDA_DEVICES})"
