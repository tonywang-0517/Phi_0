#!/usr/bin/env bash
# Single-GPU LIBERO delta-EEF ablation: ACT head only (no VLM, no cross-attn).
set -euo pipefail
...
EXP="${EXP:-experiments/libero_spatial_action_only_15k_single}"
CONFIG="${CONFIG:-train_libero_spatial_act_delta_15k_single_action_only}"
CKPT_NAME="${CKPT_NAME:-libero_spatial_action_only_15k_single}"
...
echo "==> Single-GPU action-only ablation: GPU ${CUDA_DEVICES}"
