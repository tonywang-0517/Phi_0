"""Convert Isaac-GR00T SONIC LeRobot to unified 43-d state / 100-d action format.

Output layout (pi0.5 + Phi0 training):
    states[43], action[100] = pi05_base[36] + motion_token[64]
...
from phi0.data.sonic_unified_io import (  # noqa: E402
    SONIC_ACTION_DIM,
    SONIC_STATE_DIM,
    STATS_SEMANTICS_SONIC_UNIFIED,
    build_action_from_sonic_frame,
    build_state_from_sonic_frame,
)
...
DST_LEFT_WRIST_KEY = "observation.images.left_wrist"
...
class SonicUnifiedLeRobotConverter:
    """SONIC LeRobot -> unified LeRobot (43-d state, 100-d action)."""
    ...
    def make_one_episode(
        ...
        for i in range(n):
            ...
            rows.append(
                {
                    "states": build_state_from_sonic_frame(row),
                    "action": build_action_from_sonic_frame(row),
        ...
        states = np.array([r["states"] for r in rows], dtype=np.float32)
        episode_stats = {
            "episode_index": episode_index,
            "stats": {
                "action": {
                    "min": acts.min(0).tolist(),
                    "max": acts.max(0).tolist(),
                    "mean": acts.mean(0).tolist(),
                    "std": acts.std(0).tolist(),
                    "count": [n],
                },
                "states": {
    ...
    def run(self, data_root: Path, work_dir: Path, chunks_size: int, num_workers: int) -> None:
        ...
        for pq in sorted((data_root / "data").rglob("episode_*.parquet")):
            ...
            if not video.is_file():
                video = data_root / "videos" / chunk / DST_VIDEO_KEY / ep_name
    ...
    def write_meta(self, out_dir: Path) -> None:
        ...
        features_meta = {
            ...
            "states": {"dtype": "float32", "shape": [SONIC_STATE_DIM]},
            "action": {"dtype": "float32", "shape": [SONIC_ACTION_DIM]},
    ...
    def _write_global_stats(self, meta_dir: Path) -> None:
        ...
        if not ep_stats_path.is_file():
            return
        act_sum = np.zeros(SONIC_ACTION_DIM, dtype=np.float64)
        act_sq = np.zeros(SONIC_ACTION_DIM, dtype=np.float64)
        act_min = np.full(SONIC_ACTION_DIM, np.inf)
        act_max = np.full(SONIC_ACTION_DIM, -np.inf)
        st_sum = np.zeros(SONIC_STATE_DIM, dtype=np.float64)
        st_sq = np.zeros(SONIC_STATE_DIM, dtype=np.float64)
        st_min = np.full(SONIC_STATE_DIM, np.inf)
        st_max = np.full(SONIC_STATE_DIM, -np.inf)
        total = 0
        for row in read_jsonl(ep_stats_path):
            stats = row["stats"]
            count = int(stats["action"]["count"][0])
            mean = np.asarray(stats["action"]["mean"], dtype=np.float64)
            std = np.asarray(stats["action"]["std"], dtype=np.float64)
            act_sum += mean * count
            act_sq += (std**2 + mean**2) * count
            act_min = np.minimum(act_min, np.asarray(stats["action"]["min"]))
            act_max = np.maximum(act_max, np.asarray(stats["action"]["max"]))
            sm = np.asarray(stats["states"]["mean"], dtype=np.float64)
            ss = np.asarray(stats["states"]["std"], dtype=np.float64)
            st_sum += sm * count
            st_sq += (ss**2 + sm**2) * count
            st_min = np.minimum(st_min, np.asarray(stats["states"]["min"]))
            st_max = np.maximum(st_max, np.asarray(stats["states"]["max"]))
            total += count
        if total <= 0:
            return
        act_mean = act_sum / total
        act_std = np.sqrt(np.maximum(act_sq / total - act_mean**2, 1e-12))
        st_mean = st_sum / total
        st_std = np.sqrt(np.maximum(st_sq / total - st_mean**2, 1e-12))
        stats_out = {
            "num_frames": total,
        ...
        with open(meta_dir / "stats_sonic_unified.json", "w", encoding="utf-8") as f:
...
def main() -> None:
    ...
    parser.add_argument(
        "--out-dir",
        type=Path,
        default=Path("/mnt/data2/wpy/workspace/Isaac-GR00T/data/pick_tissue_sonic_unified"),
    )
    parser.add_argument(
        "--fps",
        type=int,
        default=0,
        help="Output fps (0 = read from source meta/info.json, else 50)",
    )
    ...
    fps = int(args.fps)
    if fps <= 0:
        src_info = args.data_root / "meta" / "info.json"
        if src_info.is_file():
            fps = int(json.loads(src_info.read_text(encoding="utf-8")).get("fps", 50))
        else:
            fps = 50
    print(f"Using fps={fps}")
    conv = SonicUnifiedLeRobotConverter(fps=fps)
    conv.run(args.data_root, out_dir, args.chunks_size, args.num_workers)
    conv.write_meta(out_dir)
    print(f"Wrote unified dataset to {out_dir}")
