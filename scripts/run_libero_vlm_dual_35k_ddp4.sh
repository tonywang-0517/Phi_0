#!/usr/bin/env bash
# Ablation B: VLM + VGGT dual cross-attn + ACT, 4-GPU DDP, 35k steps, lr=2e-4, bs=16/GPU.
set -euo pipefail
...
EXP="${EXP:-experiments/libero_spatial_vlm_dual_35k_ddp4}"
CONFIG="${CONFIG:-train_libero_spatial_vlm_dual_35k_ddp4}"
CKPT_NAME="${CKPT_NAME:-libero_spatial_vlm_dual_35k_ddp4}"
...
echo "==> VLM+VGGT dual 35k DDP: ${NGPUS} GPUs (${CUDA_DEVICES})"
